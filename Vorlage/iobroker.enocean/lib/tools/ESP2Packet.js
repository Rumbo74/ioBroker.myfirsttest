...
/**
 * Possible number of buttons PTM type 1
 */
...
/**
 * Possible number of buttons PTM type 2
 */
...
/**
 * Funktion to convert any string to an byte array
 */
...
/**
 * Funktion to check the checksum
 */
...
/**
 * Funktion to check the telegram
 */
const verifyTelegram = function (buf) {
    const verified = {};
    verified.valid = true;
    // check if telegram is too short
    if (+buf.length < +14) {
        verified.headerId = HSeqTypes[98]; //tooShort
        verified.valid = false;
        return verified;
    }
    // check if telegram is too long
    if (+buf.length > +14) {
        verified.headerId = HSeqTypes[99]; //tooLong
        verified.valid = false;
        return verified;
    }
    // check if telegram starts with Sync-Bytes
    if ( buf.slice(0,2).toString("hex") != "a55a" ) {
        verified.headerId = HSeqTypes[97]; //wrongSync
        verified.valid = false;
        return verified;
    }
    return verified;
...
/**
 * Function to split a buffer into its elements 
 */
...
/**
 * Function to parse one telegram 
 */
const parse = function (buf) {
    // check main telegram
    const telegramVerify = verifyTelegram(buf);
    if (!telegramVerify.valid) {
