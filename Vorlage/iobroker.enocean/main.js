...
const ESP2Packet = require('./lib/tools/ESP2Packet').ESP2Packet;
...
class Enocean extends utils.Adapter {
	...
	async packetListenerSerial() {
		...
		SERIAL_PORT.on('open', async () => {
			if(this.config.gateway !== 'fgw14-usb') {
				...
				if (connected === true) {
					this.log.info('The serial port is connected.');
	...
	async reconnectSerial(){
		if (serialReconnectCounter <= 30){
			serialReconnectCounter += 1;
			await this.wait(60);
			this.log.info('Trying to reconnect Serial. Attempt no. ' + serialReconnectCounter);
			this.connectSerial();
		}
	}
...
// #######################
// ########################
// prüfen das serialReconnectCounter ähnlich wie TCPReconnectCounter überall angelegt ist
//          hier eine connectSerial
//          und dann ein reconnect wie unter
//          https://github.com/kylemcdonald/Highsight/blob/796377c12017624ff9916fecddeb2bb0cc72c681/Control/roboteq.js#L161
//          oder
//                      https://stackoverflow.com/questions/18551293/node-js-serialport-events-stop-working-after-reconnecting-to-port
//          oder über setInterval
...
//                      ########################
	...
	async parseMessage(data) {
		...
		this.log.debug('Packet type: ' + JSON.stringify(esp3packet.data));
		...
		switch (esp3packet.type) {
			...
			case 10: //RADIO_ERP2
				...
				this.log.debug('Packet data : ' + JSON.stringify(data));   
				const parsed = new ESP3Packet(data);
				this.log.debug('parsed Packet data : ' + JSON.stringify(parsed));  
				break;
